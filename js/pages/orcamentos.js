...
function _htmlPecasDoPedido(pedidoId, orcamentoId, editavel, podeEliminar = false) {
    ...
    const rows = pecas.map(pc => {
        ...
        const custoStock = (pesoKg && snapshot) ? pesoKg * snapshot : null;

        const plano = (DB.pecas_processos || [])
            .filter(pp => pp.pecaId === pc.id)
            .sort((a, b) => a.ordem - b.ordem);

        // Sub-linha: custo de stock
        const matLabel = mp
            ? ((mp.ref_wnr && mp.ref_wnr !== '-') ? `${mp.ref_wnr} – ${mp.ref_din}` : mp.ref_din)
            : '—';
        const stockRow = `<tr style="background:var(--color-surface-alt,#f8f8f6);">
            <td colspan="${nCols}" style="padding:0.3rem 1rem 0.3rem 1.5rem;font-size:11px;color:var(--color-text-muted);border-top:none;">
                <strong>Material:</strong> ${matLabel}
                &nbsp;|&nbsp; <strong>Peso:</strong> ${pesoKg ? pesoKg.toFixed(4) + ' kg' : '—'}
                &nbsp;|&nbsp; <strong>Preço MP:</strong> ${snapshot ? parseFloat(snapshot).toFixed(2) + ' €/kg' : '—'}
                &nbsp;|&nbsp; <strong>Custo stock:</strong> ${custoStock != null ? `<span style="font-weight:700;color:var(--color-text);">${formatEuro(custoStock)}</span>` : '—'}
            </td>
        </tr>`;

        // Sub-tabela: processos
        let processosRow;
        if (plano.length === 0) {
            processosRow = `<tr><td colspan="${nCols}" style="padding:0.4rem 1rem 0.75rem;">
                <span style="font-size:11px;color:var(--color-text-muted);font-style:italic;">Sem plano de processos definido para esta peça.</span>
            </td></tr>`;
        } else {
            ...
            processosRow = `<tr><td colspan="${nCols}" style="padding:0.4rem 1rem 0.75rem;background:var(--color-surface-alt,#f8f8f6);">
