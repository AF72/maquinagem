...
async function main() {
  ...
  const materiais = await Promise.all([
    // Aços
    prisma.materiaPrima.create({ data: { ref_wnr: '1.0503', peso_esp: 7.85, ref_din: 'C45',             ref_bs: '080M46',  ref_afnor: 'C45',        ref_une: 'C45k',        ref_aisi: '1045',  ref_jis: 'S45C',    tipo_tt: 'Têmpera e revenido' }}),
    prisma.materiaPrima.create({ data: { ref_wnr: '1.7225', peso_esp: 7.85, ref_din: '42CrMo4',         ref_bs: '708M40',  ref_afnor: '42CD4',      ref_une: '42CrMo4',     ref_aisi: '4140',  ref_jis: 'SCM440',  tipo_tt: 'Têmpera e revenido' }}),
    prisma.materiaPrima.create({ data: { ref_wnr: '1.7131', peso_esp: 7.85, ref_din: '16MnCr5',         ref_bs: '527M20',  ref_afnor: '16MC5',      ref_une: '16MnCr5',     ref_aisi: '5115',  ref_jis: 'SMnC420', tipo_tt: 'Cementação e têmpera' }}),
    prisma.materiaPrima.create({ data: { ref_wnr: '1.3505', peso_esp: 7.81, ref_din: '100Cr6',          ref_bs: '535A99',  ref_afnor: '100C6',      ref_une: 'F.131',       ref_aisi: '52100', ref_jis: 'SUJ2',    tipo_tt: 'Têmpera e revenido' }}),
    prisma.materiaPrima.create({ data: { ref_wnr: '1.0037', peso_esp: 7.85, ref_din: 'St37-2',          ref_bs: '040A10',  ref_afnor: 'A37-2',      ref_une: 'AE235-B',     ref_aisi: 'A36',   ref_jis: 'SS400',   tipo_tt: '' }}),
    prisma.materiaPrima.create({ data: { ref_wnr: '1.1191', peso_esp: 7.85, ref_din: 'C45E',            ref_bs: '080M46',  ref_afnor: 'XC45',       ref_une: 'C45E',        ref_aisi: '1045',  ref_jis: 'S45C',    tipo_tt: 'Têmpera e revenido' }}),
    prisma.materiaPrima.create({ data: { ref_wnr: '1.2379', peso_esp: 7.70, ref_din: 'X153CrMoV12',     ref_bs: 'BD3',     ref_afnor: 'Z160CDV12',  ref_une: 'X153CrMoV12', ref_aisi: 'D2',    ref_jis: 'SKD11',   tipo_tt: 'Têmpera em óleo e revenido' }}),
    prisma.materiaPrima.create({ data: { ref_wnr: '1.2311', peso_esp: 7.85, ref_din: '40CrMnMo7',       ref_bs: '-',       ref_afnor: '40CMD8',     ref_une: '40CrMnMo7',   ref_aisi: 'P20',   ref_jis: 'PX5',     tipo_tt: 'Pré-temperado (fornecido pronto)' }}),
    // Alumínios
    prisma.materiaPrima.create({ data: { ref_wnr: '3.2315', peso_esp: 2.71, ref_din: 'EN AW-6082 / AlSi1MgMn',    ref_bs: 'HE30',    ref_afnor: 'A-SGM0.7', ref_une: 'L-3341', ref_aisi: '6082', ref_jis: 'A6082', tipo_tt: 'Solubilização e envelhecimento (T6)' }}),
    prisma.materiaPrima.create({ data: { ref_wnr: '3.4365', peso_esp: 2.81, ref_din: 'EN AW-7075 / AlZn5.5MgCu',  ref_bs: 'DTD5024', ref_afnor: 'A-Z5GU',   ref_une: 'L-3372', ref_aisi: '7075', ref_jis: 'A7075', tipo_tt: 'Solubilização e envelhecimento (T651)' }}),
    prisma.materiaPrima.create({ data: { ref_wnr: '3.1355', peso_esp: 2.78, ref_din: 'EN AW-2024 / AlCu4Mg1',     ref_bs: 'L97',     ref_afnor: 'A-U4G1',   ref_une: 'L-3312', ref_aisi: '2024', ref_jis: 'A2024', tipo_tt: 'Solubilização e envelhecimento (T4/T351)' }}),
    prisma.materiaPrima.create({ data: { ref_wnr: '3.3547', peso_esp: 2.66, ref_din: 'EN AW-5083 / AlMg4.5Mn0.7', ref_bs: 'N8',      ref_afnor: 'A-G4.5MC', ref_une: 'L-3345', ref_aisi: '5083', ref_jis: 'A5083', tipo_tt: 'Sem tratamento térmico (endurecimento por deformação H111/H321)' }}),
    // Polímeros
    prisma.materiaPrima.create({ data: { ref_wnr: '-', peso_esp: 1.14, ref_din: 'PA 6.6 (ISO 1874)', ref_bs: 'BS 6089', ref_afnor: '-', ref_une: '-', ref_aisi: '-', ref_jis: 'JIS K 6920', tipo_tt: 'Sem tratamento térmico (termoplástico)' }}),
    prisma.materiaPrima.create({ data: { ref_wnr: '-', peso_esp: 1.41, ref_din: 'POM-C (ISO 9988)',  ref_bs: '-',       ref_afnor: '-', ref_une: '-', ref_aisi: '-', ref_jis: '-',          tipo_tt: 'Sem tratamento térmico (termoplástico)' }}),
    prisma.materiaPrima.create({ data: { ref_wnr: '-', peso_esp: 2.20, ref_din: 'PTFE (ISO 13000)',  ref_bs: '-',       ref_afnor: '-', ref_une: '-', ref_aisi: '-', ref_jis: '-',          tipo_tt: 'Sem tratamento térmico (termoplástico)' }}),
  ]);
  ...
  const [efacec, renova, bosch] = await Promise.all([
    prisma.empresa.create({ data: {
      nome:          'Efacec Power Solutions, S.A.',
      nif:           '500028040',
  ...
  const [carlos, ana] = await Promise.all([
    prisma.colaboradorDm.create({ data: {
  ...
  const [ruiEfacec, susanaRenova, pedroBosch] = await Promise.all([
    prisma.colaborador.create({ data: {
  ...
  const [veio, flange, tampa] = await Promise.all([
    prisma.peca.create({ data: {
      ...
      comprimento:     450.00,
      diametro_ext:    85.00,
      nota_descritiva: 'Maquinar conforme desenho. Tolerâncias H7/k6 nos apoios de rolamento.',
    }}),
    prisma.peca.create({ data: {
      ref:             'PC-2024-002',
      plano:           'DES-REN-015-B',
      ...
      diametro_int:    108.00,
      altura:          30.00,
      nota_descritiva: 'Furação conforme norma EN 1092-1 PN16. Acabamento Ra 1.6.',
    }}),
    prisma.peca.create({ data: {
      ref:             'PC-2024-003',
      plano:           'DES-BSH-007-A',
      ...
      largura:         150.00,
      altura:          8.00,
      nota_descritiva: 'Fresagem de alojamento para vedante. Anodização natural após maquinagem.',
    }}),
  ]);
  ...
  const [dpEfacec, dpRenova, dpBosch, dpJoao] = await Promise.all([
    prisma.dadosPedido.create({ data: {
      ref:             'DP-2026-001',
    ...
    prisma.dadosPedido.create({ data: {
      ...
      orgao:           'Sistema Hidráulico',
      parte:           'Circuito Primário',
      breve_descricao: 'Fabrico de flange de substituição em inox 316L',
    ...
    prisma.dadosPedido.create({ data: {
      ...
      orgao:           'Caixa de Distribuição',
      parte:           'Proteção IP54',
      breve_descricao: 'Fabrico de tampa de alumínio com alojamento para vedante',
  ...
  const [pedEfacec, pedRenova, pedBosch, pedJoao] = await Promise.all([
    prisma.pedido.create({ data: {
      ...
      observacoes:     'Urgente — transformador em avaria. Prazo máximo 10 dias úteis.',
      custo_liquido:   1850.00,
    }}),
    prisma.pedido.create({ data: {
      ref:             'PT26-0002',
      cliente_tipo:    'colaborador',
  ...
  await Promise.all([
    prisma.pecaPedido.create({ data: { peca_id: veio.id,   pedido_id: pedEfacec.id }}),
    prisma.pecaPedido.create({ data: { peca_id: flange.id, pedido_id: pedRenova.id }}),
    prisma.pecaPedido.create({ data: { peca_id: tampa.id,  pedido_id: pedBosch.id  }}),
  ]);
  ...
  const [otEfacec, otBosch] = await Promise.all([
    prisma.ordemTrabalho.create({ data: {
      num:       'OT26-0001',
  ...
  await Promise.all([
    ...
    prisma.orcamentoItem.create({ data: {
      orcamento_id:   orcRenova.id,
      item_tipo:      'peca',
      peca_id:        flange.id,
      quantidade:     1,
      valor_unitario: 575.00,
      unidade:        'un',
    }}),
    prisma.orcamentoItem.create({ data: {
      orcamento_id:   orcRenova.id,
  ...
  console.log(`  ${3} colaboradores externos  |  ${2} colaboradores internos`);
